...
public class HOTweenDemoBrain : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        colorTo.a = 0;
		sequence = new Sequence(new SequenceParms().Loops(1, LoopType.Incremental));
        ...
        //sequence.Play();*/
		Test1 = HOTween.To(CubeTrans3, 3, new TweenParms()
		                   .Prop("position", new Vector3(0, 6, 0), true) // Position tween (set as relative)
		                   .Prop("rotation", new Vector3(0, 1024, 0), true) // Relative rotation tween (this way rotations higher than 360 can be applied)
		                   //.Loops(1, LoopT) // Infinite yoyo loops
		                   .Ease(EaseType.EaseInOutQuad) // Ease
		                   );
		Test1.autoKillOnComplete = false;
		Test1.Pause();

		Seq = new Sequence (new SequenceParms ());
		Seq.Append (HOTween.To (CubeTrans3, 1, new TweenParms ().Prop ("position", new Vector3 (0, 6, 0), true)));
		Seq.Append (HOTween.To (CubeTrans4, 0.5f, new TweenParms ().Prop ("position", new Vector3 (0, 2, 0), true)));
